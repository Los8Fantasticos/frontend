...
const UserProfile = () => {
  ...
  if(!(localStorage.getItem("rolename") == null)){
    var userRole = JSON.parse(localStorage.getItem("rolename"));
  ...
  return (
    <div className="nav-item absolute right-1 top-16 bg-white dark:bg-[#42464D] p-8 rounded-lg w-96">
      ...
      <div className="flex gap-5 items-center mt-6 border-color border-b-1 pb-6">
        <div>
          <p className="font-semibold text-xl dark:text-gray-200"> {user.data.email} </p>
          <p className="text-gray-500 text-sm dark:text-gray-400">  {userRole}   </p>
          <p className="text-gray-500 text-sm font-semibold dark:text-gray-400"> {user.data.email} </p>
        </div>
      </div>
      <div>
